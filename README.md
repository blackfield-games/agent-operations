...
**Arena** — Rust workspace (`proto`, the Agent Gateway protocol; `core`, the headless reference match sim; `harness`, the loopback gateway the agent SDK plays against). The Python agent SDK lives in `agents/arena_client` and is exercised by the agents test suite:
